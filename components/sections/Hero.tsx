...
import { ArrowRight, Zap, TrendingUp, IndianRupee, Users, CheckCircle2 } from "lucide-react";
import Marquee from "react-fast-marquee";
import { useState, useEffect } from "react";

const MARQUEE_CITIES = [
  "Indore", "Jaipur", "Lucknow", "Surat", "Coimbatore", "Nagpur", "Vadodara",
  "Bhopal", "Patna", "Raipur", "Mysuru", "Visakhapatnam", "Madurai", "Amritsar",
  "Ranchi", "Dehradun", "Bhubaneswar", "Kochi", "Chandigarh", "Agra",
];
...
const CYCLING_WORDS = ["Top Investors.", "Angel Networks.", "Leading VCs.", "Deal-Makers."];

const NOTIFICATIONS = [
  { icon: "💰", text: "Bharat Angels sent an offer", sub: "₹1.2 Cr · Seed Round", color: "text-emerald-400", border: "border-emerald-500/20" },
  { icon: "👁️", text: "3 new investors viewing", sub: "KisanMart live pitch deck", color: "text-blue-400", border: "border-blue-500/20" },
  { icon: "🎉", text: "Shortlisted by KaroStartup", sub: "Shiksha360 joins Cohort 6", color: "text-[#f59e0b]", border: "border-[#f59e0b]/20" },
  { icon: "🤝", text: "Desh Capital interested", sub: "Follow-up meeting booked", color: "text-rose-400", border: "border-rose-500/20" },
];

function CyclingWord() {
  const [index, setIndex] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setIndex((i) => (i + 1) % CYCLING_WORDS.length), 2500);
...
function LivePitchCard() {
  ...
  useEffect(() => {
    const t = setInterval(() => setProgress((p) => (p >= 78 ? 42 : p + 1)), 110);
    return () => clearInterval(t);
  }, []);
  useEffect(() => {
  ...
  return (
    <div className="w-full max-w-[380px] rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.05]">
        ...
        <div className="badge-live"><div className="badge-live-dot" />LIVE</div>
      </div>
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-white">KisanMart</span>
              <span className="mono-tag text-emerald-400/70">AgriTech</span>
            </div>
            <p className="text-xs text-white/30 mt-0.5">Indore, MP &middot; Seed Round &middot; ₹1.5 Cr ask</p>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-baseline justify-between mb-1.5">
            ...
            <span className="text-xs font-mono font-medium text-emerald-400">{progress}%</span>
          </div>
          ...
          <div className="flex -space-x-1.5">
            {["RM", "PK", "AS", "SK"].map((init, idx) => (
              <div
                ...
                style={{ background: ["#f59e0b", "#3b82f6", "#10b981", "#ef4444"][idx] }}
              >
                {init}
              </div>
            ))}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="min-w-0">
              <p className={`text-[11px] font-medium ${notif.color}`}>{notif.text}</p>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 pt-32 lg:pt-40 pb-16">
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-16 xl:gap-20 items-start">
          ...
          <div>
            ...
            >
              A pitch platform built for Tier-2 and Tier-3 founders
              who deserve a seat at the table. No gatekeeping, no fluff.
            </motion.p>
        ...
        <motion.div {...fade(0.3)} className="mt-20">
          ...
          <p className="text-[11px] text-white/25 uppercase tracking-widest font-mono mb-4">Founders from 200+ cities</p>
